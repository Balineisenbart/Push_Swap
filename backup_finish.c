...
t_cheapest find_cheapest_rotation(t_node *head_to_push, t_node *head_compare)
{
    ...
    while (!(candidate->index == (current->index - 1)) && count_rx < stack_length(head_compare))  // avoid infinite loop
    {
    ...
    current = head_compare;
    while (!(candidate->index == (current->index - 1)) && count_rrx < stack_length(head_compare))
    {
